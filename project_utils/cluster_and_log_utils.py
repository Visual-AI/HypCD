...
def log_accs_from_preds_v2(y_true, y_pred, mask, eval_funcs, save_name, T=None, print_output=True, args=None):
    ...
    for i, f_name in enumerate(eval_funcs):
        ...
        if print_output:
            print_str = f'EVAL FUNCTION: {acc_f}, Epoch {T}, {log_name}: All {all_acc:.4f} | Old {old_acc:.4f} | New {new_acc:.4f}'
            # try:
            #     if dist.get_rank() == 0:
            try:
